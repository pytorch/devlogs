...
Activation checkpointing -- saving some activations and recomputing the rest, often selectively (SAC) -- is the standard policy for trading one against the other.
...
Activations are often views -- reshapes, slices, transposes -- that share storage with a base tensor rather than standalone allocations. A naive offloader can't move a view without orphaning everything else pointing at that storage, so only materialized base tensors are eligible, capping savings at about 4% of peak memory for Llama 8B.
...
H2D/D2H transfers want pinned (page-locked) host memory for full PCIe bandwidth, but allocating it per tensor via `cudaHostAlloc` costs ~3 ms each -- roughly 465 ms/step across 155 tensors on Llama3 8B, and most of the early ~50% overhead.
...
The binding is applied at the node level and is best-effort: if it cannot be established, training proceeds without it. The main purpose is to achieve maximum bandwidth -- when pinned host memory resides on the same NUMA node as the GPU, device-to-host transfers reach roughly 350 GB/s, compared with about 120 GB/s when a transfer has to cross NUMA nodes.

### Three configuration knobs for agentic experimentation: budget, defer, prefetch
...
**Phase 1 -- SAC tagging** (`memory_policy.py`): Every forward node is classified:
...
**Phase 2 -- CPU offload tagging** (`cpu_offload.py:tag_all_offloadable_activations`): The offloading pass takes the `MUST_SAVE` nodes and decides which to offload to CPU instead of keeping on GPU with the following heuristics:
...
- Research into automatic offload policies -- selecting budget, defer, and which tensors to offload without manually specified policy. Our research intern Nurlan Nazaraliyev this summer will be investigating this.

## Acknowledgements

Thanks to Sherlock Huang for the detailed design discussions and onboarding help with torchtitan and sixlib, Aditya Venkataraman for reviewing this post, and Edward Yang for initial design discussions with sixlib.
